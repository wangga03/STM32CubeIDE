#ifndef _ROS_robotic_sas_auv_ros_ObjectDetection_h
#define _ROS_robotic_sas_auv_ros_ObjectDetection_h

#include <stdint.h>
#include <string.h>
#include <stdlib.h>
#include "ros/msg.h"
#include "robotic_sas_auv_ros/BoundingBox.h"
...
namespace robotic_sas_auv_ros
{
  ...
  class ObjectDetection : public ros::Msg
  {
    public:
      ...
      typedef robotic_sas_auv_ros::BoundingBox _bounding_boxes_type;
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(bounding_boxes_lengthT > bounding_boxes_length)
        this->bounding_boxes = (robotic_sas_auv_ros::BoundingBox*)realloc(this->bounding_boxes, bounding_boxes_lengthT * sizeof(robotic_sas_auv_ros::BoundingBox));
      bounding_boxes_length = bounding_boxes_lengthT;
      for( uint32_t i = 0; i < bounding_boxes_length; i++){
      offset += this->st_bounding_boxes.deserialize(inbuffer + offset);
        memcpy( &(this->bounding_boxes[i]), &(this->st_bounding_boxes), sizeof(robotic_sas_auv_ros::BoundingBox));
      }
     return offset;
    }

    virtual const char * getType() override { return "robotic_sas_auv_ros/ObjectDetection"; };
